...
(() => {
  ...
  class Cleaner {
    constructor(adapter) {
      ...
      this.state = { documentId: globalThis.crypto.randomUUID(), revision: 0, phase: 'idle', running: false, count: 0, observed: 0, remaining: null, total: null, matching: null, threshold: 0, message: 'Ready.' };
    }
    emit(update) {
    ...
    start(threshold = 0) {
      ...
      const run = { id: ++this.sequence, threshold: percent(threshold), controller: new AbortController(), identity: null, mutationPending: false, removedIds: new Set() };
      this.run = run;
  ...
  const exports = { Cleaner, Failure, percent, watchedPercent, isWatchLater, identity, parsePage, authHeaders, delay, check };
