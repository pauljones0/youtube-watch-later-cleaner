document.addEventListener('DOMContentLoaded', async () => {
  ...
  let eventVersion = 0, commandEpoch = 0, settingsWarning = false, deleteAllArmed = false;
  const DEFAULT_THRESHOLD = C.DEFAULT_THRESHOLD ?? 90;
  let settings = { sliderThreshold: DEFAULT_THRESHOLD, advancedOpen: false };
  ...
  function settingsUI() {
    ...
    $('advancedHelp').textContent = settings.sliderThreshold > 0
      ...
      : 'All videos will be removed. Starting at 0% asks for confirmation first.';
  }
  function render(next) {
    ...
    setStatus(next.runId ? next.message : `Ready. ${settings.sliderThreshold ? `Remove videos watched at least ${settings.sliderThreshold}%.` : 'Remove ALL Watch Later videos (asks for confirmation).'}`, kind);
  ...
  async function refresh() {
    ...
    ++previewSequence; clearTimeout(previewTimer); deleteAllArmed = false;
    try {
  ...
  $('progressThreshold').addEventListener('input', () => {
    settings.sliderThreshold = C.percent($('progressThreshold').value);
    settingsUI();
    if (deleteAllArmed) { deleteAllArmed = false; if (state) render(state); }
    preview();
  });
  ...
  action.addEventListener('click', async () => {
    ...
    if (mode === 'start' && settings.sliderThreshold === 0 && !deleteAllArmed) {
      deleteAllArmed = true;
      action.textContent = 'Confirm: Delete ALL?';
      setStatus('This will remove ALL Watch Later videos. Click again to confirm, or raise the percentage to keep unwatched videos.', 'warning');
      return;
    }
    ...
    try {
      const response = await browser.tabs.sendMessage(target.id, { command, threshold: settings.sliderThreshold, confirmDeleteAll: settings.sliderThreshold === 0 });
      if (seq !== sequence || target.id !== tab?.id) return;
      busy = false;
  ...
  try {
    ...
    const rawThreshold = stored?.sliderThreshold;
    settings = { sliderThreshold: rawThreshold == null || !Number.isFinite(Number(rawThreshold)) ? DEFAULT_THRESHOLD : C.percent(rawThreshold), advancedOpen: !!stored?.advancedOpen };
